...
DEBUG = False  
VISUAL = True
expanded = [[False for j in range(6)] for i in range(8)]
visited = [[False for j in range(6)] for i in range(8)]
...
def jps(field, start_x, start_y, end_x, end_y, allowed=None):
    """
    ...
    allowed          - optional 2d array aligned with field ([row][col] = [y][x])
                        holding a direction bitmask per cell. A cardinal tendril
                        step stops when the current cell forbids that direction.
                        None preserves the old (unconstrained) behavior.
    ...
    def _peek(x, y):
        """Bounds-safe neighbour read for jump-point checks; an unpadded
        field's edge is treated the same as an OBSTACLE cell just past it."""
    ...
    def _jps_explore_diagonal (startX, startY, directionX, directionY):
        ...
        while (True):
            ...
            if not (0 <= cur_x < len(field) and 0 <= cur_y < len(field[0])):
                # ran off the edge of an unpadded field: treat like a wall.
                return None
            ...
            if _peek(cur_x + directionX, cur_y) == OBSTACLE and _peek(cur_x + directionX, cur_y + directionY) != OBSTACLE:
                return (cur_x, cur_y)
            ...
            if _peek(cur_x, cur_y + directionY) == OBSTACLE and _peek(cur_x + directionX, cur_y + directionY) != OBSTACLE:
                return (cur_x, cur_y)
    ...
    def _jps_explore_cardinal (startX, startY, directionX, directionY):
        ...
        while (True):
            if allowed is not None:
                bit = _TENDRIL_BIT[(directionX, directionY)]
                if not (allowed[cur_x][cur_y] & bit):
                    return None
            cur_x += directionX
            cur_y += directionY
            curCost += 1
            ...
            if directionX == 0:
                if _peek(cur_x + 1, cur_y) == OBSTACLE and _peek(cur_x + 1, cur_y + directionY) != OBSTACLE:
                    return cur_x, cur_y
                if _peek(cur_x - 1, cur_y) == OBSTACLE and _peek(cur_x - 1, cur_y + directionY) != OBSTACLE:
                    return cur_x, cur_y
            elif directionY == 0:
                if _peek(cur_x, cur_y + 1) == OBSTACLE and _peek(cur_x + directionX, cur_y + 1) != OBSTACLE:
                    return cur_x, cur_y
                if _peek(cur_x, cur_y - 1) == OBSTACLE and _peek(cur_x + directionX, cur_y - 1) != OBSTACLE:
                    return cur_x, cur_y

    # grid-direction bit for a tendril step; field is indexed [y][x] so
    # directionX moves grid-y and directionY moves grid-x.
    _TENDRIL_BIT = {(0, 1): 2, (0, -1): 8, (1, 0): 4, (-1, 0): 1}
...
def jps_converted_path(field,path,cur_dir):
    result = []
    if len(path)>1:
        # checkPosition 순서가 Y변화, X변화  순서임
        checkPosition = [(-1, 0, 0, 0), (-1, 1, 1, 1), (0, 1, 2, 2), (1, 1, 3, 3), (1, 0, 4, 4), (1, -1, 5, 5),
                         (0, -1, 6, 6), (-1, -1, 7, 7)]

        result.append([path[0],cur_dir,None])

        for idx in range(len(path)-1):
            for samplePosition in checkPosition:
                next_y = path[idx][0] + samplePosition[0]
                next_x = path[idx][1] + samplePosition[1]
                next_dir = samplePosition[2]
                next_action = samplePosition[3]

                if next_y < 0  or next_y > (len(field) - 1): continue
                if next_x < 0  or next_x > (len(field[len(field)-1])-1): continue

                if (next_y,next_x) == path[idx+1]:
                    result.append([path[idx+1],next_dir,next_action])
                    break
        return result
    else: return None
