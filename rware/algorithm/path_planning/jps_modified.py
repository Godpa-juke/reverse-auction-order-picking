...
# expanded = [[False for j in range(300)] for i in range(400)]
# visited = [[False for j in range(300)] for i in range(400)]
...
def jps(field, start_x, start_y, end_x, end_y, dir):
    """
    ...
    def _jps_explore_diagonal (startX, startY, directionX, directionY):
        ...
        while (True):
            ...
            if field [cur_x + directionX] [cur_y + directionY] != OBSTACLE: return (cur_x, cur_y)
    ...
    def _jps_explore_cardinal (startX, startY, directionX, directionY):
        ...
        while (True):
            ...
            if directionX == 0:
                ...
                # if field [cur_x - 1] [cur_y] == OBSTACLE and field [cur_x - 1] [cur_y + directionY] != OBSTACLE:
                #     return cur_x, cur_y
                if field [cur_x] [cur_y + directionY] != OBSTACLE: return cur_x, cur_y
                ...
                # if field [cur_x] [cur_y - 1] == OBSTACLE and field [cur_x + directionX] [cur_y - 1] != OBSTACLE:
                #     return cur_x, cur_y
                if field [cur_x + directionX] [cur_y] != OBSTACLE: return cur_x, cur_y
    # MAIN JPS FUNCTION
    ...
    while (not pq.empty()):
        ...
        try:
            ...
            for adder in [0, 1, -1, 2, -2, 3, -3, 4]:
                num = ((8 + dir) + adder)%8
                if num % 2 == 0:
                    queue_jumppoint(_jps_explore_cardinal(pX, pY, delta_x[num], delta_y[num]))
                else:
                    queue_jumppoint(_jps_explore_diagonal(pX, pY, delta_x[num], delta_y[num]))
...
def jps_converted_path(field,path,cur_dir,diagonal):
    result = []
    if len(path) > 1:
        ...
        for idx in range(len(path) - 1):
            for samplePosition in checkPosition:
                cur_y  = path[idx][0]
                cur_x  = path[idx][1]
                next_y = cur_y + samplePosition[0]
                next_x = cur_x + samplePosition[1]
                ...
                if (next_y, next_x) == path[idx + 1]:
                    if next_dir == 1:
                        if field[cur_y - 1][cur_x] == 0:
                            result.append([(cur_x,cur_y-1), 0, 0])
                            break
                        elif field[cur_y][cur_x+1] == 0:
                            result.append([(cur_x+1,cur_y), 2, 2])
                            break

                    elif next_dir == 3:
                        if field[cur_y+1][cur_x] == 0:
                            result.append([(cur_x, cur_y+1), 4, 4])
                            break
                        elif field[cur_y][cur_x+1] == 0:
                            result.append([(cur_x+1, cur_y), 2, 2])
                            break
                        ...
                        if field[cur_y][cur_x - 1] == 0:
                            result.append([(cur_x - 1, cur_y), 6, 6])
                            break

                        elif field[cur_y + 1][cur_x] == 0:
                            result.append([(cur_x, cur_y + 1), 4, 4])
                            break
